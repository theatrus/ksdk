...
/*
 * Message send from WebSocket to plugin handler
 */
typedef struct ws_plugin_msg
{
    /* Socket to be inserted back to receive FD_SET. */
    uint32_t sock;
}WS_PLUGIN_MSG;
...
typedef struct ws_init_struct
{
    ...
    /* Buffer size. */
    uint32_t          buf_len;
    /* Task ID of WebScoket task. */
    volatile _task_id *tid;
    /* Message queue for communication with WebSocket session. */
    volatile void**   msg_queue;
}WS_INIT_STRUCT;

/*
 * WebSocket plugin context.
 */
typedef struct ws_plugin_context
{
    ...
    uint32_t          sock_count;
    /* WebSocket task Ids. */
    volatile _task_id tids[RTCSCFG_FD_SETSIZE];
    /* Array of all sockets. */
    uint32_t          socks[RTCSCFG_FD_SETSIZE];
    /* Message queues. */
    volatile void*    queues[RTCSCFG_FD_SETSIZE];
    /* Queue for receiving of messages from WebSocket session. */
    void*             recv_queue;
}WS_PLUGIN_CONTEXT;
