...
/**
 * @brief Check family of given socket.
 *
 * Check if socket has right family, depending on enabled IP protocols.
 *  
 * @param[in] sock Pointer to @c SOCKET_STRUCT structure (socket handle).
 *  
 * @return FALSE if socket family is not enabled.
 * @return TRUE if socket family is enabled.
 */
bool RTCS_socket_check_family(uint32_t sock)
{
  ...
  if(
      ...
      (AF_INET == socket_ptr->AF)
      #if RTCSCFG_ENABLE_IP6
      ||
      #endif
    #endif
    #if RTCSCFG_ENABLE_IP6
      (AF_INET6 == socket_ptr->AF)
    #endif
    )
  {
    retval = TRUE;
