...
{ /* Body */
   ...
   if((NULL == optlen) || (*optlen < sizeof(uint32_t)))
   {
...
{ /* Body */
   ...
   {
     RTCS_EXIT(SETSOCKOPT, RTCSERR_SOCK_SHORT_OPTION);
   }
#endif
...
{ 
   ...
   SOCKET_STRUCT_PTR sock_ptr = ((SOCKET_STRUCT_PTR)parms->ucb->SOCKET);

   switch (parms->udpflags) 
   {
    case RTCS_SO_UDP_NOCHKSUM: /* = OPT_CHECKSUM_BYPASS */
      parms->udpword = parms->ucb->BYPASS_TX;
      break;
   
    case RTCS_SO_UDP_NONBLOCK_TX: /* = OPT_SEND_NOWAIT */
    case RTCS_SO_UDP_NONBLOCK_RX: /* = OPT_RECEIVE_NOWAIT */
    case SO_SNDTIMEO: /* = OPT_SEND_TIMEOUT */
    case SO_RCVTIMEO: /* = OPT_RECEIVE_TIMEOUT */
     parms->udpword = ((uint32_t *)&(sock_ptr->CONNECT_TIMEOUT))[parms->udpflags-1];
     break;
...
{ 
  ...
  switch (parms->udpflags) 
  {
    ...
    case RTCS_SO_UDP_NONBLOCK_TX: /* = OPT_SEND_NOWAIT */
    case RTCS_SO_UDP_NONBLOCK_RX: /* = OPT_RECEIVE_NOWAIT */
      ((uint32_t *)&(sock_ptr->CONNECT_TIMEOUT))[parms->udpflags-1] = parms->udpword;
      break;
