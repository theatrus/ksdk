...
void ftpsrv_server_task(void* init_ptr, void* creator)
{
    ...
    while (server->run)
    {
        ...
        while ((!conn_sock) && server->run)
        {
            ...
            retval = select(2, &rfds, 0, 250);

            if((0 == retval) || (RTCS_ERROR == retval))
            {
              continue;
            }
            if(RTCS_FD_ISSET(server->sock_v4, &rfds))
            {
                conn_sock = server->sock_v4;
            }
            else if(RTCS_FD_ISSET(server->sock_v6, &rfds))
            {
                conn_sock = server->sock_v6;
            }
        }
        ...
        if(new_sock != RTCS_SOCKET_ERROR)
        {
            ...
            res = RTCS_task_create(FTPSRV_SESSION_TASK_NAME, server->params.server_prio, FTPSRV_SESSION_STACK_SIZE, ftpsrv_session_task, &ses_param);
            if (MQX_OK != res)
            {
