...
static uint32_t ws_process_api_calls(WS_CONTEXT_STRUCT *context, void **msg_queue);
...
uint32_t ws_init(WS_INIT_STRUCT *init_params, WS_CONTEXT_STRUCT **context_out)
{
    ...
    *context_out = context;
    ...
    context->msg_queue = RTCS_mem_alloc_zero(sizeof(LWMSGQ_STRUCT) + WS_NUM_MESSAGES*sizeof(WS_API_CALL_MSG)*sizeof(_mqx_max_type));
    if 
        (
            (context->msg_queue == NULL) ||
            (MQX_OK != _lwmsgq_init(context->msg_queue, WS_NUM_MESSAGES, sizeof(WS_API_CALL_MSG)/sizeof(_mqx_max_type)))
        )
    {
        return(WS_ERR_FAIL);
    }
...
static uint32_t ws_process(WS_CONTEXT_STRUCT* context)
{
    ...
    if (ws_process_api_calls(context, &msg_queue) != WS_ERR_PASS)
    {
        retval = WS_ERR_FAIL;
        goto EXIT;
    }
...
EXIT:
    /* Signal back to handler task, that data processing on socket is done. */
    if (msg_queue != 0)
    {
        WS_PLUGIN_MSG message;

        message.sock = context->sock; 
        _lwmsgq_send(msg_queue, (_mqx_max_type *) &message, LWMSGQ_SEND_BLOCK_ON_FULL);
    }
EXIT2:
    return(retval);
}
...
static uint32_t ws_process_api_calls(WS_CONTEXT_STRUCT *context, void **msg_queue)
{
    ...
    while (MQX_OK == _lwmsgq_receive(context->msg_queue, (_mqx_max_type *) &message, LWMSGQ_TIMEOUT_FOR | LWMSGQ_RECEIVE_BLOCK_ON_EMPTY, 0, NULL))
    {
        ...
        switch (message.command)
        {
            ...
            case WS_COMMAND_CLOSE:
                ...
                _task_ready(_task_get_td((_task_id) message.data));
                break;
            case WS_COMMAND_RECV:
                /* 
                 * Read frame from client, if required.
                 */
                if (ws_recv(context) != WS_ERR_PASS)
                {
                    retval = WS_ERR_FAIL;
                }
                *msg_queue = message.data;
                break;
            default:
                break;
        }
...
static void ws_deinit(WS_CONTEXT_STRUCT *context)
{
    WS_PLUGIN_STRUCT*           plugin = context->plugin;

    /* Call user disconnect callback */
    plugin->on_disconnect(plugin->cookie, context->u_context);

    if (context->msg_queue != NULL)
    {
        _lwmsgq_deinit(context->msg_queue);
        _mem_free(context->msg_queue);
    }
    ...
    shutdown(context->sock, FLAG_CLOSE_TX);
    _mem_free(context);
}
