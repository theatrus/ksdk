...
static TFTPSRV_SESSION_STRUCT* tftpsrv_ses_alloc(TFTPSRV_STRUCT *server)
{
    ...
    if (server)
    {
        ...
        if (session)
        {
            ...
            session->buffer = _mem_alloc_zero(TFTP_MAX_MESSAGE_SIZE);
            if (session->buffer != NULL)
            {
                _mem_set_type(session, MEM_TYPE_TFTPSRV_SESSION_BUFFER);
            }
            else
            {
                _mem_free(session);
...
static uint32_t tftpsrv_ses_init(TFTPSRV_STRUCT* server, TFTPSRV_SESSION_STRUCT *session, uint32_t sock)
{
    ...
    retval = recvfrom(sock, session->buffer, TFTP_MAX_MESSAGE_SIZE, 0, &remote_sin, &size);
    if (retval == RTCS_ERROR)
    {
        goto FAIL;
    }
    ...
    session->sock = socket(family, SOCK_DGRAM, 0);
    if (session->sock == 0)
    {
        goto FAIL;
    }
    ...
    if (connect(session->sock, &remote_sin, sizeof(remote_sin)) != RTCS_OK)
    {
        goto FAIL;
    }

    session->client_sin = remote_sin;
    session->valid = 1;
...
void tftpsrv_transaction_prologue(void* server_v, void *session_v)
{
    ...
    error = tftpsrv_open_file(opcode, filename, root_dir, mode, &session->file);
    if (error != TFTPSRV_OK)
    {
        tftpsrv_error(session, error, TFTPSRV_FATAL);
    }
    else
    {
    ...
    {
        session->state = TFTPSRV_STATE_SEND_DATA;
        session->n_block = 1;
    }
    else if (opcode == TFTP_OPCODE_WRQ)
    {
        session->state = TFTPSRV_STATE_SEND_ACK;
        session->n_block = 0;
    }
}
...
static void tftpsrv_process_transaction(void* server_v, void *session_v)
{
    ...
    {
        case TFTPSRV_STATE_SEND_DATA:
            data_length = tftpsrv_send_data(session);
            ...
            {
                session->state = TFTPSRV_STATE_READ_ACK;
            }
            break;
        case TFTPSRV_STATE_READ_DATA:
            data_length = tftpsrv_recv_data(session);
            if (data_length == RTCS_ERROR)
            {
                session->process_func = tftpsrv_transaction_epilogue;
            }
            ...
            break;
        case TFTPSRV_STATE_READ_ACK:
            data_length = tftpsrv_recv_ack(session);
            if ((data_length == RTCS_ERROR) || session->last)
            {
                session->process_func = tftpsrv_transaction_epilogue;
            }
            else if (data_length >= 0)
            ...
            break;
        case TFTPSRV_STATE_SEND_ACK:
            data_length = tftpsrv_send_ack(session);
            if ((data_length == RTCS_ERROR) || session->last)
            {
                session->process_func = tftpsrv_transaction_epilogue;
            }
            ...
            {
                session->state = TFTPSRV_STATE_READ_DATA;
            }
            break;
        default:
            break;
