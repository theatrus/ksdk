...
static uint32_t tftp_wait(uint32_t timeout_ms);
...
static uint32_t tftp_wait(uint32_t timeout_ms)
{
  rtcs_fd_set rfds;
  int32_t err;
  uint32_t retval = 0; /* default = timeout */
  RTCS_FD_ZERO(&rfds);
  RTCS_FD_SET(TFTP_config.SOCK, &rfds);

  err = select(1, &rfds, NULL, timeout_ms);
  if(RTCS_ERROR == err)
  {
