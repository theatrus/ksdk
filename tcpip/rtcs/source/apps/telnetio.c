...
#else // !MQX_USE_IO_OLD
static int _io_telnet_open(void *dev_context, const char *dev_name, int flags, void **fp_context);
static int _io_telnet_read(void *dev_context, void *fp_context, void *buf, size_t nbytes);
static int _io_telnet_write(void *dev_context, void *fp_context, const void *buf, size_t nbytes);
static int _io_telnet_ioctl(void *dev_context, void *fp_context, unsigned long int request, va_list ap);
static int _io_telnet_close(void *dev_context, void *fp_context);
static int _io_telnet_init(void *init_data, void **dev_context);
static int _io_telnet_deinit(void *dev_context);
...
{
  //IO_TELNET *io_ptr = (IO_TELNET*)dev_context;
...
{ /* Body */
   ...
   io_ptr = fd_ptr->DEV_DATA_PTR;
#else // !MQX_USE_IO_OLD   
static int _io_telnet_read(void *dev_context, void *fp_context, 
                           void *data_ptr, size_t num) 
{
...
#else
static int _io_telnet_ioctl(void *dev_context, void *fp_context, 
                            unsigned long int cmd, va_list ap) 
{
    IO_TELNET * io_ptr = (IO_TELNET *)fp_context;
    int32_t         result;
#if MQX_USE_IO_OLD
...
_mqx_int _io_telnet_process_char (char *cp, void *fp_context)
{
  ...
  FILE * console_stream;
   ...
   case 3: /* IAC DO */
      ...
      case TELOPT_BINARY:
          /* ACK only */
          if (!io_ptr->LOCAL_OPTIONS[(int) c]) {
             io_ptr->LOCAL_OPTIONS[(int) c] = TRUE;
   ...
   case 5: /* IAC DONT */
      ...
      if (c < TELNET_MAX_OPTS) {
         /* ACK only */
         if (io_ptr->LOCAL_OPTIONS[(int) c]) {
            io_ptr->LOCAL_OPTIONS[(int) c] = FALSE;
            TELNET_wont(io_ptr->NETWORK_FD, c);
         } /* Endif */
      } /* Endif */
