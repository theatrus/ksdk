   ...
   )
{
  ...
  { 
    ...
    retval = select(echo_ptr->num_listensock
                    ...
                    + echo_ptr->num_clients, &rfds, 0, 0);

    if(RTCS_ERROR == retval)
    {
      if(RTCSERR_SOCK_CLOSED == RTCS_errno)
      {
        _task_set_error(RTCS_OK); /* clear task error */
        /*
        ...
        /*
         * other task closes listening sockets. 
         * We interpret this as an application command to stop server & sessions.
         */
        for(i=0; i<echo_ptr->num_listensock; i++)
        {
...
uint32_t ECHOSRV_release(void* server_h)
{
  ...
  {
    shutdown(echo_context_ptr->listensock[i], FLAG_CLOSE_TX);
  }
