...
static void Udp_ReturnRequestToSocketLayer(RTCSPCB_PTR pcb_ptr, UDP_PARM_PTR parms);
      ...
      */
      if (!b_ip_is_zero) {
         ...
         RTCS_LIST_SEARCH(UDP_cfg_ptr->BOUND_UCB_HEAD, search_ptr) {
            if((SOCKADDR_get_port(&search_ptr->LADDR) == port) && (((SOCKET_STRUCT_PTR)search_ptr->SOCKET)->AF == parms->saddr_ptr->sa_family)) {
               RTCSCMD_complete(parms, RTCSERR_UDP_PORT_OPEN);
               return;
            } /* Endif */
         } /* End SEARCH */
   ...
   )
{
  ...
  uint16_t remote_port = SOCKADDR_get_port(parms->saddr_ptr);
  uint16_t raddr_port = SOCKADDR_get_port(&ucb->RADDR);
...
{
  UDP_PARM_PTR parms = event->PRIVATE;
  RTCS_QUEUE_DEL(parms->ucb->RHEAD, parms->ucb->RTAIL);
...
{
  RTCSPCB_PTR       pcb_ptr;
  bool nonblock_rx = (((uint32_t)TRUE == ((SOCKET_STRUCT_PTR)parms->ucb->SOCKET)->RECEIVE_NOWAIT)
                           || (parms->udpflags & RTCS_MSG_PEEK));
  uint32_t timeout_ms = ((SOCKET_STRUCT_PTR)parms->ucb->SOCKET)->RECEIVE_TIMEOUT;
  /*
  ...
  */
  ...
  if (pcb_ptr) 
  {
    ...
    */
    Udp_ReturnRequestToSocketLayer(pcb_ptr, parms);    
...
{ /* Body */
   ...
   if (parms->udpflags & RTCS_MSG_O_NONBLOCK) {
      ...
      nonblock = ((SOCKET_STRUCT_PTR)parms->ucb->SOCKET)->SEND_NOWAIT == (uint32_t)TRUE;
   } /* Endif */
   ...
   if((((SOCKET_STRUCT_PTR)(parms->ucb->SOCKET))->AF) == AF_INET6)
   {
     if(((sockaddr_in6*)(parms->saddr_ptr))->sin6_scope_id)
        ...
        else if(((sockaddr_in6*)&parms->ucb->LADDR)->sin6_scope_id)
        {/* parms->ucb->IF_SCOPE_ID has value of scope_id from local address.
          * If it is not NULL we will use it for calculation what interface we will be used for send packet.
          */
            pcb->IFSRC = ip6_if_get_by_scope_id(((sockaddr_in6*)&(parms->ucb->LADDR))->sin6_scope_id);
        }
        else
        {   /*
    ...
    error = UDP_send_internal(parms->ucb, &(parms->ucb->LADDR), parms->saddr_ptr, pcb, parms->udpflags);
...
#if RTCSCFG_ENABLE_IP4
    #if RTCSCFG_ENABLE_IP6
    uint16_t             af = ((SOCKET_STRUCT_PTR)ucb->SOCKET)->AF;
    #endif
#endif
...
{ /* Body */
   ...
   UDP_HEADER_PTR packet;
   uint32_t        error;
   uint16_t  src_port = SOCKADDR_get_port(&ucb->LADDR);
...
{ /* Body */
   ...
   */
   ...
   if(req_ptr)
   {
      ...
      Udp_ReturnRequestToSocketLayer(pcb_ptr, req_ptr); 
...
static void Udp_ReturnRequestToSocketLayer(RTCSPCB_PTR pcb_ptr, UDP_PARM_PTR parms)
{
    if(parms->saddr_ptr)
    {
      ...
      parms->saddr_ptr->sa_family = af;
    #if RTCSCFG_ENABLE_IP4
      #if RTCSCFG_ENABLE_IP6
      if(af == AF_INET)      
      {
      #endif
        SOCKADDR_init((void *)IP_source(pcb_ptr), UDP_source(pcb_ptr), parms->saddr_ptr);
      #if RTCSCFG_ENABLE_IP6
      }
      #endif
    #endif
      ...
      {
      #endif      
        SOCKADDR_init((in6_addr *)(((IP6_HEADER_PTR)(pcb_ptr->NETWORK_LAYER))->SOURCE), UDP_source(pcb_ptr), parms->saddr_ptr);
        /* here we get scope_id */
        ((sockaddr_in6*)(parms->saddr_ptr))->sin6_scope_id = RTCS6_if_get_scope_id((_rtcs_if_handle)pcb_ptr->IFSRC);
      #if RTCSCFG_ENABLE_IP4
      }
      #endif
    #endif
