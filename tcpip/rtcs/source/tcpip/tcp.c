...
static int32_t tcb_to_time_wait(TCB_STRUCT_PTR tcb, TCP_CFG_STRUCT_PTR tcp_cfg, uint32_t rtcslog_p3)
{
  ...
  /* cancel FIN-WAIT-2 timeout. TIME_WAIT timeout starts. */
  /* tcb->EXPIER.PRIVATE is non-null only by a user call to TCP_Process_release(). in which case the FIN-WAIT-2 timeout starts. */
...
/*
 * @internal
 * FIN-WAIT-2 timeout has expired.
 */
static bool tcb_finwait2_expire(TCPIP_EVENT_PTR event)
{
  TCB_STRUCT_PTR tcb = event->PRIVATE;
  TCP_CFG_STRUCT_PTR   tcp_cfg;          /* TCP Layer constants */
  
  if(
      (tcb->state == FINWAIT_2) && 
      (tcb->SOCKET == 0) &&      /* socket struct for the TCB has beed freed. by RTCS_shutdown(). */
...
{ /* Body */
   ...
   uint16_t          af = ((SOCKET_STRUCT_PTR)tcb->SOCKET)->AF;;
#if RTCSCFG_ENABLE_IP6
         ...
         if ( (flags & SYN) != 0 ) {                         /* THIRD */
            ...
            if(SOCKADDR_ip_is_zero(&tcb->raddr))
            {
              ...
              if(AF_INET == af)
              {
                ...
                tcb->rcvmss = IP_MTU(SOCKADDR_get_ipaddr4(&tcb->laddr), SOCKADDR_get_ipaddr4(&srchost)) - (uint16_t) (IP_HEADSIZE + TCP_HEADSIZE);
              #if RTCSCFG_ENABLE_IP6
              }
              #endif
#endif
              ...
              else if(AF_INET6 == af)
              {
                ...
                tcb->rcvmss = IP6_MTU(SOCKADDR_get_ipaddr6(&tcb->laddr) , &ipv6_source) - (uint16_t) (IP6_HEADSIZE + TCP_HEADSIZE);
              #if RTCSCFG_ENABLE_IP4
              }
              #endif
#endif
                  ...
                  case FINWAIT_1:
                     ...
                     */
                     if ( GT32(ack, tcb->sndbufseq + tcb->sndlen) ) 
                     {
                       ...
                       tcb->expire.EVENT = tcb_finwait2_expire;
