...
static int nio_sdcard_open(void *dev_context, const char *dev_name, int flags, void **fp_context);
static int nio_sdcard_close(void *dev_context, void *fp_context);
static int nio_sdcard_ioctl(void *dev_context, void *fp_context, unsigned long int cmd, va_list ap);
static int nio_sdcard_read(void *dev_context, void *fp_context, void *data_ptr, size_t n);
static int nio_sdcard_write(void *dev_context, void *fp_context, const void *data_ptr, size_t n);
static int nio_sdcard_deinit(void *dev_context);
static int nio_sdcard_init(void *init_data, void **dev_context);
static _nio_off_t nio_sdcard_lseek(void *dev_context, void *fp_context, _nio_off_t offset, int whence);
 ...
 *
 * \return SDCARD_OK on success, SDCARD_ERR or -EINVAL on error
 */
 ...
 *
 * \return Location in bytes on success, -EINVAL on error
 */
static _nio_off_t nio_sdcard_lseek(void *dev_context, void *fp_context, _nio_off_t offset, int whence)
{
  SDCARD_STRUCT_PTR    sdcard_ptr = (SDCARD_STRUCT_PTR)dev_context;
 ...
 *
 * \return Returns the number of bytes received on success, SDCARD_ERR, -ENXIO or -EIO on error (negative)
 */
    ...
    )
{
    ...
    _lwsem_wait(&sdcard_ptr->LWSEM);
    res = sdcard_ptr->INIT.READ_FUNC (sdcard_ptr, (unsigned char *)data_ptr,
                                       sdcard_ptr->LOCATION>>IO_SDCARD_BLOCK_SIZE_POWER, num>>IO_SDCARD_BLOCK_SIZE_POWER);

    if (res > 0)
    {
        sdcard_ptr->LOCATION += res;
    }
 ...
 *
 * \return Returns number of bytes transmitted on success, SDCARD_ERR, -ENOSPC or -EIO on error (negative)
 */
    ...
    )
{
    ...
    _lwsem_wait(&sdcard_ptr->LWSEM);
    res = sdcard_ptr->INIT.WRITE_FUNC (sdcard_ptr, (unsigned char *)data_ptr,
                                        sdcard_ptr->LOCATION>>IO_SDCARD_BLOCK_SIZE_POWER, num>>IO_SDCARD_BLOCK_SIZE_POWER);
