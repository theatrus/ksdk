...
const SDCARD_CONST_INIT_STRUCT sdcard_esdhc_init_data = { 
 ...
 *
 * \return SDCARD_OK on success, SDCARD_ERR on error, -ENODEV if device does not support high speed mode
 */
...
#define SFS_GET_BYTE_CNT(bit) (63 - (bit) / 8)
  ...
  if((cmd6_data[SFS_GET_BYTE_CNT(SFS_FUNC_GROUP1_BIT)] & 0x01) == 0)
    return -ENODEV; //  if (bit 401 is '0') report the SD card does not support high speed mode and return;
 ...
 *
 * \return Number of read bytes on success, SDCARD_ERR or -ETIMEDOUT on error
 */
 ...
 *
 * \return Number of written bytes on success, SDCARD_ERR or -ETIMEDOUT on error
 */
