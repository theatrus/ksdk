...
static int nio_esdhc_open(void *dev_context, const char *dev_name, int flags, void **fp_context);
static int nio_esdhc_close(void *dev_context, void *fp_context);
static int nio_esdhc_ioctl(void *dev_context, void *fp_context, unsigned long int cmd, va_list ap);
static int nio_esdhc_read(void *dev_context, void *fp_context, void *data_ptr, size_t n);
static int nio_esdhc_write(void *dev_context, void *fp_context, const void *data_ptr, size_t n);
static int nio_esdhc_deinit(void *dev_context);
static int nio_esdhc_init(void *init_data, void **dev_context);
...
const NIO_DEV_FN_STRUCT nio_esdhc_dev_fn =
{
    .OPEN = nio_esdhc_open,
    .READ = nio_esdhc_read,
    .WRITE = nio_esdhc_write,
    .LSEEK = NULL,
    .IOCTL = nio_esdhc_ioctl,
    .CLOSE = nio_esdhc_close,
    .INIT = nio_esdhc_init,
    .DEINIT = nio_esdhc_deinit,
};
...
{
    uint32_t   esdhc_base = esdhc_device_ptr->DEV_BASE;
    uint32_t clock;
    ...
    HW_SDHC_SYSCTL_CLR(esdhc_base, BM_SDHC_SYSCTL_SDCLKEN);

    /* Change dividers */
    div = HW_SDHC_SYSCTL_RD(esdhc_base) & (~(BM_SDHC_SYSCTL_DTOCV | BM_SDHC_SYSCTL_SDCLKFS | BM_SDHC_SYSCTL_DVS));
    HW_SDHC_SYSCTL_WR(esdhc_base, div | BF_SDHC_SYSCTL_DTOCV(0x0E) | BF_SDHC_SYSCTL_SDCLKFS(minpres >> 1) | BF_SDHC_SYSCTL_DVS(mindiv - 1));


    /* Wait for stable clock */
    while (0 == (HW_SDHC_PRSSTAT_RD(esdhc_base) & BM_SDHC_PRSSTAT_SDSTB))
    {
        _sched_yield();
    }

    /* Enable ESDHC clocks */
    HW_SDHC_SYSCTL_SET(esdhc_base, BM_SDHC_SYSCTL_SDCLKEN);

    HW_SDHC_IRQSTAT_SET(esdhc_base, BM_SDHC_IRQSTAT_DTOE);
    ...
    )
{
    ...
    /* Get total divider */
    div = BR_SDHC_SYSCTL_SDCLKFS(esdhc_base) << 1;
    div *= BR_SDHC_SYSCTL_DVS(esdhc_base) + 1;
...
{
    return (0 != (HW_SDHC_PRSSTAT_RD(esdhc_base) & (BM_SDHC_PRSSTAT_RTA | BM_SDHC_PRSSTAT_WTA | BM_SDHC_PRSSTAT_DLA | BM_SDHC_PRSSTAT_CDIHB | BM_SDHC_PRSSTAT_CIHB)));
}
...
static int nio_esdhc_init
    (
        ...
        void *init_data, void **dev_context
    )
{
    ESDHC_DEVICE_STRUCT_PTR    esdhc_device_ptr;
    ...
    )
{
    ...
    /* Reset ESDHC */
    HW_SDHC_SYSCTL_WR(esdhc_base, BF_SDHC_SYSCTL_RSTA(1) | BF_SDHC_SYSCTL_SDCLKFS(0x80));

    while (HW_SDHC_SYSCTL_RD(esdhc_base) & BM_SDHC_SYSCTL_RSTA)
    { ; }

    /* Just to keep card detection working */
    HW_SDHC_SYSCTL_WR(esdhc_base, BM_SDHC_SYSCTL_PEREN);

    /* Initial values */
    HW_SDHC_VENDOR_WR(esdhc_base, 0); /* Switch off the external DMA requests */

    HW_SDHC_BLKATTR_WR(esdhc_base, BF_SDHC_BLKATTR_BLKCNT(1) | BF_SDHC_BLKATTR_BLKSIZE(512)); /* Set any default size of data block */

#if PSP_ENDIAN == MQX_BIG_ENDIAN
    HW_SDHC_PROCTL_WR(esdhc_base, BF_SDHC_PROCTL_EMODE(0) | BF_SDHC_PROCTL_D3CD(1) | BF_SDHC_PROCTL_DMAS(2));
#else
    HW_SDHC_PROCTL_WR(esdhc_base, BF_SDHC_PROCTL_EMODE(2) | BF_SDHC_PROCTL_D3CD(1) | BF_SDHC_PROCTL_DMAS(2));
#endif

    HW_SDHC_WML_WR(esdhc_base, BF_SDHC_WML_RDWML(1) | BF_SDHC_WML_WRWML(1));  /* To do: set up the right value */

    /* Set the ESDHC initial baud rate divider and start */
    _esdhc_set_baudrate(esdhc_device_ptr, ESDHC_INIT_BAUDRATE);

    /* Poll inhibit bits */
    while (HW_SDHC_PRSSTAT_RD(esdhc_base) & (BM_SDHC_PRSSTAT_CIHB | BM_SDHC_PRSSTAT_CDIHB))
    { ; }

    /* Enable requests */
    HW_SDHC_IRQSTAT_WR(esdhc_base, 0xFFFF);

    HW_SDHC_IRQSTATEN_WR(esdhc_base,    \
     /*DMA Error Status Enable*/   BF_SDHC_IRQSTATEN_DMAESEN(1)  | \
     /*Auto CMD12 Error Status Enable*/   BF_SDHC_IRQSTATEN_AC12ESEN(1) | \
     /*Data End Bit Error Status Enable*/   BF_SDHC_IRQSTATEN_DEBESEN(1)  | \
     /*Data CRC Error Status Enable*/   BF_SDHC_IRQSTATEN_DCESEN(1)   | \
     /*Data Timeout Error Status Enable*/   BF_SDHC_IRQSTATEN_DTOESEN(1)  | \
     /*Command Index Error Status Enable*/   BF_SDHC_IRQSTATEN_CIESEN(1)   | \
     /*Command End Bit Error Status Enable*/   BF_SDHC_IRQSTATEN_CEBESEN(1)  | \
     /*Command CRC Error Status Enable*/   BF_SDHC_IRQSTATEN_CCESEN(1)   | \
     /*Command Timeout Error Status Enable*/   BF_SDHC_IRQSTATEN_CTOESEN(1)  | \
     /*Card Interrupt Status Disable*/   BF_SDHC_IRQSTATEN_CINTSEN(0)  | \
     /*Card Removal Status Enable*/   BF_SDHC_IRQSTATEN_CRMSEN(1)   | \
     /*Card Insertion Status Enable*/   BF_SDHC_IRQSTATEN_CINSEN(1)   | \
     /*Buffer Read Ready Status Disable*/   BF_SDHC_IRQSTATEN_BRRSEN(0)   | \
     /*Buffer Write Ready Status Disable*/   BF_SDHC_IRQSTATEN_BWRSEN(0)   | \
     /*DMA Interrupt Status Enable*/   BF_SDHC_IRQSTATEN_DINTSEN(1)  | \
     /*Block Gap Event Status Enable*/   BF_SDHC_IRQSTATEN_BGESEN(1)   | \
     /*Transfer Complete Status Enable*/   BF_SDHC_IRQSTATEN_TCSEN(1)    | \
     /*Command Complete Status Enable*/   BF_SDHC_IRQSTATEN_CCSEN(1)      \
         );


    /* 80 initial clocks */
    HW_SDHC_SYSCTL_SET(esdhc_base, BM_SDHC_SYSCTL_INITA);
    while (HW_SDHC_SYSCTL_RD(esdhc_base) & BM_SDHC_SYSCTL_INITA)
    { ; }

    /* Check card */
    if(HW_SDHC_IRQSTAT_RD(esdhc_base) & BM_SDHC_IRQSTAT_CINS)  //!(BR_SDHC_IRQSTAT_CINS(esdhc_base))
    {
    ...
    HW_SDHC_IRQSIGEN_WR(esdhc_base,     \
       /*DMA Error Status Enable*/  BF_SDHC_IRQSIGEN_DMAEIEN(1)  |  \
       /*Auto CMD12 Error Status Enable*/  BF_SDHC_IRQSIGEN_AC12EIEN(1) |  \
       /*Data End Bit Error Status Enable*/  BF_SDHC_IRQSIGEN_DEBEIEN(1)  |  \
       /*Data CRC Error Status Enable*/  BF_SDHC_IRQSIGEN_DCEIEN(1)   |  \
       /*Data Timeout Error Status Enable*/  BF_SDHC_IRQSIGEN_DTOEIEN(1)  |  \
       /*Command Index Error Status Enable*/  BF_SDHC_IRQSIGEN_CIEIEN(1)   |  \
       /*Command End Bit Error Status Enable*/  BF_SDHC_IRQSIGEN_CEBEIEN(1)  |  \
       /*Command CRC Error Status Enable*/  BF_SDHC_IRQSIGEN_CCEIEN(1)   |  \
       /*Command Timeout Error Status Enable*/  BF_SDHC_IRQSIGEN_CTOEIEN(1)  |  \
       /*Card Interrupt Status Disable*/  BF_SDHC_IRQSIGEN_CINTIEN(0)  |  \
       /*Card Removal Status Enable*/  BF_SDHC_IRQSIGEN_CRMIEN(1)   |  \
       /*Card Insertion Status Enable*/  BF_SDHC_IRQSIGEN_CINSIEN(1)  |  \
       /*Buffer Read Ready Status Disable*/  BF_SDHC_IRQSIGEN_BRRIEN(0)   |  \
       /*Buffer Write Ready Status Disable*/  BF_SDHC_IRQSIGEN_BWRIEN(0)   |  \
       /*DMA Interrupt Status Enable*/  BF_SDHC_IRQSIGEN_DINTIEN(1)  |  \
       /*Block Gap Event Status Enable*/  BF_SDHC_IRQSIGEN_BGEIEN(1)   |  \
       /*Transfer Complete Status Enable*/  BF_SDHC_IRQSIGEN_TCIEN(1)    |  \
       /*Command Complete Status Enable*/  BF_SDHC_IRQSIGEN_CCIEN(1)       \
        );
#else
    HW_SDHC_IRQSIGEN_WR(esdhc_base,     \
        /*DMA Error Status Enable*/ BF_SDHC_IRQSIGEN_DMAEIEN(1)  |  \
        ...
        /*Command Timeout Error Status Enable*/ BF_SDHC_IRQSIGEN_CTOEIEN(1)  |  \
        /*Card Interrupt Status Disable*/ BF_SDHC_IRQSIGEN_CINTIEN(0)  |  \
        /*Card Removal Status Disable*/ BF_SDHC_IRQSIGEN_CRMIEN(0)   |  \
        /*Card Insertion Status Disable*/  BF_SDHC_IRQSIGEN_CINSIEN(0)  |  \
        /*Buffer Read Ready Status Disable*/ BF_SDHC_IRQSIGEN_BRRIEN(0)   |  \
        /*Buffer Write Ready Status Disable*/ BF_SDHC_IRQSIGEN_BWRIEN(0)   |  \
        /*DMA Interrupt Status Enable*/ BF_SDHC_IRQSIGEN_DINTIEN(1)  |  \
        /*Block Gap Event Status Enable*/ BF_SDHC_IRQSIGEN_BGEIEN(1)   |  \
    ...
    )
{
    ...
    if(command->COMMAND & ESDHC_COMMAND_ACMD_FLAG)
    {
        ...
        xfertyp = BF_SDHC_XFERTYP_CMDINX(55) | BF_SDHC_XFERTYP_CICEN(1) | BF_SDHC_XFERTYP_CCCEN(1) | BF_SDHC_XFERTYP_RSPTYP(2);

        /* Wait for cmd line idle */
        while (HW_SDHC_PRSSTAT_RD(esdhc_base) & BM_SDHC_PRSSTAT_CIHB)
        { ; }

        /* Setup command */
        HW_SDHC_CMDARG_WR(esdhc_base, 0);
        HW_SDHC_BLKATTR_WR(esdhc_base, 0);
        ...
        HW_SDHC_XFERTYP_WR(esdhc_base, xfertyp);
    ...
    xfertyp = BF_SDHC_XFERTYP_CMDINX(command->COMMAND & ESDHC_COMMAND_CMDIX_MASK);
    ...
    if(((command->COMMAND & ESDHC_COMMAND_CMDTYPE_MASK) == ESDHC_COMMAND_TYPE_RESUME) || (command->COMMAND & ESDHC_COMMAND_DATACMD_FLAG))
      xfertyp |= BF_SDHC_XFERTYP_DPSEL(1) | BF_SDHC_XFERTYP_DMAEN(1);
    ...
    if ((0 != command->BLOCKS) && (0 != command->BLOCKSIZE))
    {
        ...
        if (command->BLOCKS != 1)
        {
            ...
            xfertyp |= BF_SDHC_XFERTYP_MSBSEL(1);

            if (command->BLOCKS > 1)
              xfertyp |= BF_SDHC_XFERTYP_AC12EN(1) | BF_SDHC_XFERTYP_BCEN(1);
        }
        if ((uint32_t)-1 == command->BLOCKS)
        {
            /* Infinite transfer */
            blkattr = BF_SDHC_BLKATTR_BLKSIZE(command->BLOCKSIZE) | BF_SDHC_BLKATTR_BLKCNT(0xFFFF);
            xfertyp |= BF_SDHC_XFERTYP_BCEN(1);
        }
        else
        {
            blkattr = BF_SDHC_BLKATTR_BLKSIZE(command->BLOCKSIZE) | BF_SDHC_BLKATTR_BLKCNT(command->BLOCKS);
        }
    }
    else
    {
    ...
    if((xfertyp & BF_SDHC_XFERTYP_DPSEL(1)) && (command->COMMAND & ESDHC_COMMAND_DATA_READ_FLAG))
        xfertyp  |= BF_SDHC_XFERTYP_DTDSEL(1);

    /* Set up response type */
    if(command->COMMAND & (ESDHC_COMMAND_RESPONSE_R1 | ESDHC_COMMAND_RESPONSE_R5 | ESDHC_COMMAND_RESPONSE_R6))
        xfertyp |= BF_SDHC_XFERTYP_CICEN(1) | BF_SDHC_XFERTYP_CCCEN(1) | BF_SDHC_XFERTYP_RSPTYP(2);
    else if(command->COMMAND & ESDHC_COMMAND_RESPONSE_R2)
        xfertyp |= BF_SDHC_XFERTYP_CCCEN(1) | BF_SDHC_XFERTYP_RSPTYP(1);
    else if(command->COMMAND & (ESDHC_COMMAND_RESPONSE_R3 | ESDHC_COMMAND_RESPONSE_R4))
        xfertyp |= BF_SDHC_XFERTYP_RSPTYP(2);
    else if(command->COMMAND & (ESDHC_COMMAND_RESPONSE_R1b | ESDHC_COMMAND_RESPONSE_R5b))
        xfertyp |= BF_SDHC_XFERTYP_CICEN(1) | BF_SDHC_XFERTYP_CCCEN(1) | BF_SDHC_XFERTYP_RSPTYP(3);

    /* Set up the command type */
    xfertyp |= BF_SDHC_XFERTYP_CMDTYP((command->COMMAND & ESDHC_COMMAND_CMDTYPE_MASK) >> ESDHC_COMMAND_CMDTYPE_SHIFT);
    ...
    */
    ...
    if((temp != 0) && (temp != 12) && (temp != 13) && (temp != 52))
    {
        ...
        HW_SDHC_SYSCTL_SET(esdhc_base, BF_SDHC_SYSCTL_PEREN(1) | BF_SDHC_SYSCTL_HCKEN(1) | BF_SDHC_SYSCTL_IPGEN(1));

        _time_get_ticks(&start_tick);

        while (HW_SDHC_PRSSTAT_RD(esdhc_base) & BM_SDHC_PRSSTAT_CDIHB)
        {
            _time_get_ticks(&end_tick);
    ...
    HW_SDHC_SYSCTL_CLR(esdhc_base, BF_SDHC_SYSCTL_PEREN(1) | BF_SDHC_SYSCTL_HCKEN(1) | BF_SDHC_SYSCTL_IPGEN(1));

    /* Setup command */
    HW_SDHC_DSADDR_WR(esdhc_base, 0);
    HW_SDHC_ADSADDR_WR(esdhc_base, (uint32_t)adma2_desc_ptr);
    HW_SDHC_BLKATTR_WR(esdhc_base, blkattr);
    HW_SDHC_CMDARG_WR(esdhc_base, command->ARGUMENT);
    ...
    {
        command->RESPONSE[0] = HW_SDHC_CMDRSP0_RD(esdhc_base);
        if ((xfertyp & BM_SDHC_XFERTYP_RSPTYP) == BF_SDHC_XFERTYP_RSPTYP(1))
        {
            command->RESPONSE[1] = HW_SDHC_CMDRSP1_RD(esdhc_base);
            command->RESPONSE[2] = HW_SDHC_CMDRSP2_RD(esdhc_base);
            command->RESPONSE[3] = HW_SDHC_CMDRSP3_RD(esdhc_base);
        }
    }
    ...
    if((xfertyp & (BF_SDHC_XFERTYP_RSPTYP(3) | BM_SDHC_XFERTYP_AC12EN)) == (BF_SDHC_XFERTYP_RSPTYP(3) | BM_SDHC_XFERTYP_AC12EN))
        HW_SDHC_SYSCTL_SET(esdhc_base, BF_SDHC_SYSCTL_PEREN(1) | BF_SDHC_SYSCTL_HCKEN(1) | BF_SDHC_SYSCTL_IPGEN(1));
    ...
    )
{
    ...
    if (HW_SDHC_IRQSTAT_RD(esdhc_base) & BM_SDHC_IRQSTAT_CINS)
    {
        ...
        while (!(BR_SDHC_PRSSTAT_DLSL(esdhc_base) & 0x1))
        {
            _time_get_ticks(&end_tick);
    ...
    )
{
    ...
    switch (cmd)
    {
        ...
        case IO_IOCTL_ESDHC_GET_BAUDRATE_MAX:
            ...
            {
                if(HW_SDHC_HTCAPBLT_RD(esdhc_base) & BM_SDHC_HTCAPBLT_HSS) //!(BR_SDHC_HTCAPBLT_HSS(esdhc_base))
        ...
        case IO_IOCTL_ESDHC_SET_BAUDRATE:
            ...
            {
                if (!_esdhc_is_running (esdhc_base))
                {
        ...
        case IO_IOCTL_ESDHC_GET_BUS_WIDTH:
            ...
            else
            {
                ...
                val = BR_SDHC_PROCTL_DTW(esdhc_base);
            ...
            else
            {
                ...
                if (!_esdhc_is_running (esdhc_base))
                {
                    if (ESDHC_BUS_WIDTH_1BIT == *param32_ptr)
                    {
                        ...
                        BW_SDHC_PROCTL_DTW(esdhc_base, 0/*ESDHC_PROCTL_DTW_1BIT*/);
                    }
                    else if (ESDHC_BUS_WIDTH_4BIT == *param32_ptr)
                    {

                        BW_SDHC_PROCTL_DTW(esdhc_base, 1/*ESDHC_PROCTL_DTW_4BIT*/);
                    }
                    else if (ESDHC_BUS_WIDTH_8BIT == *param32_ptr)
                    {

                        BW_SDHC_PROCTL_DTW(esdhc_base, 2/*ESDHC_PROCTL_DTW_8BIT*/);
                    }
                    else
                    {
                        result = ESDHC_ERROR_INVALID_BUS_WIDTH;
        ...
        case IO_IOCTL_ESDHC_GET_CARD:
            ...
            else
            {
                ...
                HW_SDHC_SYSCTL_SET(esdhc_base, BF_SDHC_SYSCTL_INITA(1)); //! BW_SDHC_SYSCTL_INITA(esdhc_base, 1);
                while (HW_SDHC_SYSCTL_RD(esdhc_base) & BM_SDHC_SYSCTL_INITA) //! (BR_SDHC_SYSCTL_INITA(esdhc_base))
                { ; }

                /* Update and return actual card status */
                if (HW_SDHC_IRQSTAT_RD(esdhc_base) & BM_SDHC_IRQSTAT_CRM) //! (BR_SDHC_IRQSTAT_CRM(esdhc_base))
                {
                    HW_SDHC_IRQSTAT_SET(esdhc_base, BF_SDHC_IRQSTAT_CRM(1)); //! BW_SDHC_IRQSTAT_CRM(esdhc_base, 1);
                    esdhc_device_ptr->CARD = ESDHC_CARD_NONE;
                }
        ...
        case IO_IOCTL_ESDHC_SET_IO_CALLBACK:
            if (NULL == param_ptr || ((ESDHC_IO_INT_CALLBACK_STRUCT *)param_ptr)->CALLBACK == NULL)
            {
                ...
                HW_SDHC_IRQSIGEN_CLR(esdhc_base, BM_SDHC_IRQSIGEN_CINTIEN); //! BW_SDHC_IRQSIGEN_CINTIEN(esdhc_base, 0);
                HW_SDHC_IRQSTAT_SET(esdhc_base, BF_SDHC_IRQSTAT_CINT(1)); //! BW_SDHC_IRQSTAT_CINT(esdhc_base, 1);
            ...
            else
            {
                ...
                HW_SDHC_IRQSTAT_SET(esdhc_base, BF_SDHC_IRQSTAT_CINT(1)); //! BW_SDHC_IRQSTAT_CINT(esdhc_base, 1);
                HW_SDHC_IRQSIGEN_SET(esdhc_base, BF_SDHC_IRQSIGEN_CINTIEN(1)); //! BW_SDHC_IRQSIGEN_CINTIEN(esdhc_base, 1);
            }
            break;
    ...
    )
{
    ...
    else
    {
       if(_lwevent_get_signalled() & ESDHC_LWEVENT_TRANSFER_ERROR)
       {
            if(esdhc_device_ptr->BUFFERED_CMD.BLOCKS > 1)
            {
                ...
                int32_t count = (int32_t)((uint32_t)HW_SDHC_ADSADDR_RD(esdhc_base) - (uint32_t)data_ptr);
                ...
                HW_SDHC_SYSCTL_SET(esdhc_base, BF_SDHC_SYSCTL_RSTD(1)); //!BW_SDHC_SYSCTL_RSTD(esdhc_base, 1);
                return count;
            }
            else
            {
    ...
    if (!(BR_SDHC_PRSSTAT_DLSL(esdhc_base) & 0x1))
        HW_SDHC_SYSCTL_CLR(esdhc_base, BF_SDHC_SYSCTL_PEREN(1) | BF_SDHC_SYSCTL_HCKEN(1) | BF_SDHC_SYSCTL_IPGEN(1));
    ...
    )
{
  ...
  sdhc_irqstat = HW_SDHC_IRQSTAT_RD(esdhc_base);
  sdhc_irqsigen = HW_SDHC_IRQSIGEN_RD(esdhc_base);
  // Clear the all sets IRQ status bits
  HW_SDHC_IRQSTAT_WR(esdhc_base, sdhc_irqstat);
  ...
  */
  if ((sdhc_irqsigen & BM_SDHC_IRQSIGEN_DMAEIEN)&&(sdhc_irqstat & BM_SDHC_IRQSTAT_DMAE))
  {
  ...
  */
  if ((sdhc_irqsigen & BM_SDHC_IRQSIGEN_AC12EIEN)&&(sdhc_irqstat & BM_SDHC_IRQSTAT_AC12E))
  {
  ...
  */
  if ((sdhc_irqsigen & BM_SDHC_IRQSIGEN_DEBEIEN)&&(sdhc_irqstat & BM_SDHC_IRQSTAT_DEBE))
  {
    _lwevent_set( &esdhc_device_ptr->LWEVENT, ESDHC_LWEVENT_TRANSFER_ERROR);
  }
  ...
  */
  if ((sdhc_irqsigen & BM_SDHC_IRQSIGEN_DCEIEN)&&(sdhc_irqstat & BM_SDHC_IRQSTAT_DCE))
  {
    _lwevent_set( &esdhc_device_ptr->LWEVENT, ESDHC_LWEVENT_TRANSFER_ERROR);
  }
  ...
  /*
    ...
    Occurs when detecting one of following time-out conditions.
      ...
      • Read Data time-out
  */
  if ((sdhc_irqsigen & BM_SDHC_IRQSIGEN_DTOEIEN)&&(sdhc_irqstat & BM_SDHC_IRQSTAT_DTOE))
  {
    _lwevent_set( &esdhc_device_ptr->LWEVENT, ESDHC_LWEVENT_TRANSFER_ERROR);
  }
  ...
  */
  if ((sdhc_irqsigen & BM_SDHC_IRQSIGEN_CIEIEN)&&(sdhc_irqstat & BM_SDHC_IRQSTAT_CIE))
  {
  ...
  */
  if ((sdhc_irqsigen & BM_SDHC_IRQSIGEN_CEBEIEN)&&(sdhc_irqstat & BM_SDHC_IRQSTAT_CEBE))
  {
    _lwevent_set( &esdhc_device_ptr->LWEVENT, ESDHC_LWEVENT_CMD_ERROR);
  }
  ...
  */
  if ((sdhc_irqsigen & BM_SDHC_IRQSIGEN_CCEIEN)&&(sdhc_irqstat & BM_SDHC_IRQSTAT_CCE))
  {
    _lwevent_set( &esdhc_device_ptr->LWEVENT, ESDHC_LWEVENT_CMD_ERROR);
  }
  ...
  */
  if ((sdhc_irqsigen & BM_SDHC_IRQSIGEN_CTOEIEN)&&(sdhc_irqstat & BM_SDHC_IRQSTAT_CTOE))
  {
  ...
  */
  if ((sdhc_irqsigen & BM_SDHC_IRQSIGEN_CINTIEN)&&(sdhc_irqstat & BM_SDHC_IRQSTAT_CINT))
  {
  ...
  /*
    ...
    Status Enable register.
      ...
      1b Card removed
  */
  if ((sdhc_irqsigen & BM_SDHC_IRQSIGEN_CRMIEN)&&(sdhc_irqstat & BM_SDHC_IRQSTAT_CRM))
  {
    // Set card to NONE
    esdhc_device_ptr->CARD = ESDHC_CARD_NONE;

    // Switch of Card Insert interrupts
    HW_SDHC_IRQSIGEN_CLR(esdhc_base, BM_SDHC_IRQSIGEN_CRMIEN); //!BW_SDHC_IRQSIGEN_CRMIEN(esdhc_base, 0);
    HW_SDHC_IRQSIGEN_SET(esdhc_base, BF_SDHC_IRQSIGEN_CINSIEN(1)); //!BW_SDHC_IRQSIGEN_CINSIEN(esdhc_base, 1);

    // Switch the peripheral to 1 bit mode to enable correct work of Dat3 signal to card insert detection

    BW_SDHC_PROCTL_DTW(esdhc_base, ESDHC_PROCTL_DTW_1BIT);
  ...
  /*
    ...
    Enable register.
      ...
      1b Card inserted
  */
  else if ((sdhc_irqsigen & BM_SDHC_IRQSIGEN_CINSIEN)&&(sdhc_irqstat & BM_SDHC_IRQSTAT_CINS))
  {
    // Switch off Card Insert interrupts


    HW_SDHC_IRQSIGEN_CLR(esdhc_base, BM_SDHC_IRQSIGEN_CINSIEN); //!BW_SDHC_IRQSIGEN_CINSIEN(esdhc_base, 0);
    HW_SDHC_IRQSIGEN_SET(esdhc_base, BF_SDHC_IRQSIGEN_CRMIEN(1)); //!BW_SDHC_IRQSIGEN_CRMIEN(esdhc_base, 1);
  ...
  */
  if ((sdhc_irqsigen & BM_SDHC_IRQSIGEN_BRRIEN)&&(sdhc_irqstat & BM_SDHC_IRQSTAT_BRR))
  {
  ...
  */
  if ((sdhc_irqsigen & BM_SDHC_IRQSIGEN_BWRIEN)&&(sdhc_irqstat & BM_SDHC_IRQSTAT_BWR))
  {
  ...
  */
  if ((sdhc_irqsigen & BM_SDHC_IRQSIGEN_DINTIEN)&&(sdhc_irqstat & BM_SDHC_IRQSTAT_DINT))
  {
  ...
  */
  if ((sdhc_irqsigen & BM_SDHC_IRQSIGEN_BGEIEN)&&(sdhc_irqstat & BM_SDHC_IRQSTAT_BGE))
  {
    //_lwevent_set( &esdhc_device_ptr->LWEVENT, ESDHC_LWEVENT_TRANSFER_ERROR);
  }
  ...
  */
  if ((sdhc_irqsigen & BM_SDHC_IRQSIGEN_TCIEN)&&(sdhc_irqstat & BM_SDHC_IRQSTAT_TC))
  {
    _lwevent_set( &esdhc_device_ptr->LWEVENT, ESDHC_LWEVENT_TRANSFER_DONE);
  }
  ...
  /*
    ...
    PRSSTAT[CIHB].
  */
  if ((sdhc_irqsigen & BM_SDHC_IRQSIGEN_CCIEN)&&(sdhc_irqstat & BM_SDHC_IRQSTAT_CC))
  {
