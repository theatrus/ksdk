...
MFS_HANDLE_PTR MFS_Open_file
    (
        ...
        uint32_t             fsflags,    /*[IN] type of access required: read, write or read/write*/
        _mfs_error          *error_ptr   /*[IN/OUT] error code is written to this address */
    )
{
    ...
    MFS_HANDLE_PTR           existing_handle = NULL;
    bool                     write_access;
    ...
    if ((fsflags & MFS_O_CREAT) && (fsflags & MFS_O_EXCL)) {
        ...
        if ( error_code == MFS_NO_ERROR )
        {
            ...
            else if ( (mqx_dtohc(dir_entry_ptr->ATTRIBUTE) & MFS_ATTR_READ_ONLY) && ((fsflags & MFS_O_ACCMODE) != MFS_O_RDONLY) )
            {
                error_code = MFS_ACCESS_DENIED;
            }
            else
            {
                ...
                existing_handle = MFS_Find_handle(drive_ptr, dir_cluster, dir_index);
