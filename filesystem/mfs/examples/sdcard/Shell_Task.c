...
{ /* Body */
   ...
   _nio_dev_install("esdhc:", &nio_esdhc_dev_fn, (void*)&_bsp_esdhc_init);
   ...
   /* install device */
   if (_nio_dev_install("sdcard:", &nio_sdcard_dev_fn, (void*)&sdcard_init_data) == NULL)
   {
   ...
   for(;;){
     ...
     if(last != inserted){
       ...
       else
       {
         ...
         error_code = _nio_dev_uninstall_force("a:");
         if (error_code != MFS_NO_ERROR)
         {
         ...
         error_code = _nio_dev_uninstall_force(partman_name);
         if (error_code != MFS_NO_ERROR)
         {
