...
int mfs_unmount(int fs_fd_ptr, char *fs_name, bool force)
{
    ...
    if (*fs_name) {
      ...
      /* Uninstall MFS */
      if (force)
          error_code = _nio_dev_uninstall_force(fs_name);
      else
          error_code = _nio_dev_uninstall(fs_name);
    ...
    )
{
    ...
    } while ((EEXIST == errno) && (dev_id < (ELEMENTS_OF(dev_name) - 1)));
...
*/
    ...
    } while (MQX_OK != error_code && EEXIST == errno && pm_id < (ELEMENTS_OF(pm_name) - 1));
    ...
    if ((error_code != MQX_OK) || (valid_parts == 0))
    {
        ...
        error_code = _nio_dev_uninstall(msd_dev_data->PM_NAME);
        if (error_code != MFS_NO_ERROR)
        {
    ...
    )
{
    ...
    if (msd_dev_data->PM_NAME && *(msd_dev_data->PM_NAME)) {
        if (force)
            error_code = _nio_dev_uninstall_force(msd_dev_data->PM_NAME);
        else
            error_code = _nio_dev_uninstall(msd_dev_data->PM_NAME);
    }
    ...
    if (msd_dev_data->DEV_NAME && *(msd_dev_data->DEV_NAME)) {
        if (force)
            error_code = _nio_dev_uninstall_force(msd_dev_data->DEV_NAME);
        else
            error_code = _nio_dev_uninstall(msd_dev_data->DEV_NAME);
    }
